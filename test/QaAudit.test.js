...
// The seventeen shipped member class names (WindowFold added at v1.7.0). This list is the
...
// ===========================================================================
// Cross-file "seventeen members" regression guard. v1.7.0 grew the roster to
// seventeen (WindowFold); this test fails if a future session adds/removes a member
// from O1.js without also updating README.md / GUIDE.md / llms.txt, or vice
// versa (docs claim seventeen but the module does not export seventeen).
// ===========================================================================

test('O1.js exports exactly the seventeen frozen members plus VERSION -- no more, no fewer', () => {
    for (const name of SEVENTEEN_MEMBERS) {
    ...
    const expected = [...SEVENTEEN_MEMBERS, 'VERSION'].sort();
    assert.deepEqual(exportedNames, expected, 'O1.js export surface drifted from the frozen seventeen-member + VERSION list');
});

test('README.md, GUIDE.md, and llms.txt all describe the roster as "seventeen members" (case-insensitive), never a stale count', () => {
    const files = ['README.md', 'GUIDE.md', 'llms.txt'];
    for (const f of files) {
        const text = readFileSync(join(ROOT, f), 'utf8');
        assert.match(text, /seventeen members?/i, f + ' must describe the roster as "seventeen member(s)" somewhere');
        ...
        // nine (pre-HTW), eleven (pre-CuckooMap), twelve (pre-SparseTable), and sixteen
        // (pre-WindowFold), plus the stale 72-cell benchmark size.
...
test('every SEVENTEEN_MEMBERS name appears in the GUIDE.md picker table and decision flowchart', () => {
