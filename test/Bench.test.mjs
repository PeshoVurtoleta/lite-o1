...
test('adoption: SUBJECTS is 15; cells() is 120; each new member has exactly 8 cells', () => {
    assert.equal(SUBJECTS.length, 15, 'SUBJECTS must be the 15 shipped members');
    for (const m of NEW_MEMBERS) assert.ok(SUBJECTS.includes(m), m + ' must be registered');
    const all = cells();
    assert.equal(all.length, 120, 'grid must be 15 x 8 = 120 cells');
...
test('shipping discipline -- package.json.version is the 1.5.0 bump; benchmark/ stays repo-only', () => {
    const pkg = JSON.parse(readFileSync(new URL('../package.json', import.meta.url), 'utf8'));
    assert.equal(pkg.version, '1.5.0', '1.5.0 appends AliasTable (the fifteenth member)');
...
test('#1 CLEAR_WITNESS is EXACTLY the four container members; each has a clear()', () => {
    ...
    assert.equal(excl.length, 11, 'eleven members excluded with reasons');
...
test('#6 trinity + shipping surface: VERSION 1.5.0 across O1.js/package.json/llms.txt; benchmark/ not shipped', () => {
    const pkg = JSON.parse(readFileSync(new URL('../package.json', import.meta.url), 'utf8'));
    assert.equal(VERSION, '1.5.0', 'O1.js VERSION const');
    assert.equal(pkg.version, '1.5.0', 'package.json version');
    const m = LLMS.match(/^Version:\s*(\S+)/m);
    assert.ok(m, 'llms.txt Version header present');
    assert.equal(m[1], '1.5.0', 'llms.txt Version header');
    ...
    assert.equal(cells().length, 120, 'bench grid stays 15 x 8 = 120 cells');
});
