...
test('shipping discipline -- package.json.version is the 1.4.1 bump; benchmark/ stays repo-only', () => {
    const pkg = JSON.parse(readFileSync(new URL('../package.json', import.meta.url), 'utf8'));
    assert.equal(pkg.version, '1.4.1', '1.4.1 hotfix reverts the BitSet.firstSet leak probe');
...
test('#6 trinity + shipping surface: VERSION 1.4.1 across O1.js/package.json/llms.txt; benchmark/ not shipped', () => {
    const pkg = JSON.parse(readFileSync(new URL('../package.json', import.meta.url), 'utf8'));
    assert.equal(VERSION, '1.4.1', 'O1.js VERSION const');
    assert.equal(pkg.version, '1.4.1', 'package.json version');
    const m = LLMS.match(/^Version:\s*(\S+)/m);
    assert.ok(m, 'llms.txt Version header present');
    assert.equal(m[1], '1.4.1', 'llms.txt Version header');
