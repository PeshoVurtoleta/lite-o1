// @zakkster/lite-o1 -- demo static file server (repo-only dev artifact, NEVER shipped).
//
//   npm run demo:serve            (then open http://localhost:8020/)
//   node demo/serve.mjs [port]
//
// A zero-dependency Node http server (node:http + node:fs only). Its web root is the LiteO1
// REPO ROOT, so the page at /demo/index.html and its `../O1.js` + `./kernels.mjs` imports all
// resolve from one origin. There is no dynamic route -- lite-o1's demo drives the REAL shipped
// classes in the browser directly (no Node-side workload to hand over, unlike lite-filter), so
// this server only serves static files and fails CLOSED on every bad path (null -> 404).
//
...
const ROOT = dirname(DEMO_DIR); // repo root -- so `../O1.js` from /demo resolves to /O1.js
...
export async function handle(req, res) {
    ...
    // (../O1.js, ./kernels.mjs) would resolve wrong. Redirecting makes the browser re-request
...
export function start(port) {
    ...
    server.listen(port, () => {
        process.stdout.write('lite-o1 demo server on http://localhost:' + port + '/\n');
