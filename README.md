...
> Zero-GC, O(1) data structures that WITNESS their constant. v1.11.1 ships SparseSet (an integer set with O(1) add / has / delete / iterate and an O(1) clear() that zeroes nothing), RingDeque (a fixed-capacity numeric double-ended queue with O(1) push/pop at both ends), UnionFind (a disjoint-set forest with near-O(1) amortized find / union), MonoDeque (a monotonic deque for O(1)-amortized sliding-window min / max), MinStack (a fixed-capacity numeric stack with a worst-case-O(1) running min / max), RandomSet (an integer set with worst-case-O(1) uniform sample / removeRandom), FreqO1 (a worst-case-O(1) LFU frequency structure with O(1) add / increment / peekMin / popMin), BucketQueue (an amortized-O(1) monotone integer priority queue / Dial with O(1) insert / decreaseKey / extractMin), TimerWheel (a worst-case-O(1) bounded simple timing wheel with O(1) schedule / cancel / advance and drain-before-advance), HierarchicalTimerWheel (an amortized-O(1) cascading multi-level timing wheel with a 2^26 delay range), RingLog (a worst-case-O(1) lossy overwrite-oldest ring log whose O(1) push returns the evicted oldest), CuckooMap (a bounded-probe worst-case-O(1)-lookup exact map from general integer keys to numbers via bucketized cuckoo hashing), SparseTable (a worst-case-O(1)-query STATIC range-minimum / range-maximum table / StaticRMQ), BitSet (a fixed-capacity multi-word DENSE bitset with worst-case-O(1) test / set / unset / toggle and worst-case-O(1) firstSet / nextSet via a 3-level popcount summary), AliasTable (a worst-case-O(1) STATIC Vose weighted sampler drawing an outcome index by weight after an O(n) build), CoarseTimerWheel (a worst-case-O(1) near-unbounded non-cascading timing wheel that fires far-future timers in place at an approximate, bounded-late time), WindowFold (a worst-case-O(1) general FIFO sliding-window aggregator over SUM / MIN / MAX / PRODUCT via DABA-Lite), RankSelect (a worst-case-O(1) STATIC succinct rank / select bitvector index over a frozen bit pattern, cs-poppy class), EliasFano (a worst-case-O(1)-access STATIC succinct codec for a monotone integer sequence in ~2 + log2(U/n) bits/element), Reservoir (a worst-case-O(1)-per-item exact uniform k-sampler over an unbounded stream via Vitter's Algorithm R), and WindowFoldUint32 (a worst-case-O(1) bitwise OR / AND / XOR sliding-window fold over 32-bit masks on the DABA-Lite core) -- plus a throughput-invariance witness that shows the flat cost curve while a native Set, Array.prototype.shift, a naive disjoint-set, a full-window rescan, a full-stack rescan, a Set-iterate-to-the-kth, a frequency-table min-scan, a binary heap, a naive-scan scheduler, a 4-ary heap, a shift-on-full Array log, a naive linear-scan map, a naive O(len) range-scan, a cache-degrading Set, a naive O(n) cumulative-scan sampler, a naive O(W) window refold, an O(words) popcount scan, a from-scratch stream resample, or a naive O(W) bitwise-window refold decays.
...
v1.11.1 ships twenty-one members. **SparseSet**, the textbook O(1) integer set (a dense + sparse array pair) whose `clear()` runs in O(1) by resetting a count and zeroing nothing at all. **RingDeque**, a fixed-capacity double-ended queue of numbers over one circular `Float64Array` -- O(1) push/pop at both ends, the zero-GC answer to the `Array.prototype.shift` O(n) trap. **UnionFind**, a disjoint-set forest over two `Uint32Array` columns -- near-O(1) amortized `find` / `union` via path halving + union by size, the family's first amortized-honesty member. **MonoDeque**, a monotonic deque over two parallel `Float64Array` columns -- O(1)-amortized sliding-window min / max, the zero-GC answer to the full-window-rescan O(W) trap. **MinStack**, a fixed-capacity numeric stack over two parallel `Float64Array` columns (value + a running-extreme prefix) -- WORST-CASE O(1) push/pop plus a running min / max, no amortization asterisk. **RandomSet**, SparseSet's substrate plus WORST-CASE O(1) uniform `sample()` / `removeRandom()` -- the zero-GC answer to the `Array.from(set)[k]` O(n)-plus-allocation trap. **FreqO1**, a WORST-CASE O(1) frequency structure over a private bucket forest -- `add` / `increment` / `peekMin` / `popMin`, the standalone primitive behind O(1) LFU eviction, the zero-GC answer to the scan-all-counts-for-the-minimum O(n) trap. **BucketQueue**, an AMORTIZED O(1) monotone integer priority queue ("Dial") over private key columns + a static per-priority bucket array -- `insert` / `decreaseKey` / `extractMin`, the standalone primitive behind Dial's algorithm, the zero-GC answer to a binary heap's O(log n) per op when priorities are small bounded integers. And **TimerWheel**, a WORST-CASE O(1) bounded "simple" timing wheel (Varghese-Lauck) over private id columns + a static per-slot FIFO ring -- `schedule` / `cancel` / `drainDue` / `advance`, the standalone primitive behind O(1) timer scheduling, the zero-GC answer to a binary-heap timer queue's O(log n) per op (and a linear scan's O(n) per tick) when the delay horizon is bounded. And **HierarchicalTimerWheel**, an AMORTIZED O(1) CASCADING multi-level timing wheel (the Linux tvec shape: 1x256 + 3x64, delay range 2^26) over the same substrate plus a Float64 expiry column -- `schedule` / `cancel` / `drainDue` / `advance`, TimerWheel's sibling for a delay horizon too wide for one rotation, cascading coarse timers down to finer levels by index (zero allocation) and wearing an honest max-single-op cascade spike. And **RingLog**, a WORST-CASE O(1) LOSSY overwrite-oldest ring log over one circular `Float64Array` -- "keep the last N": `push` never blocks and never throws on full, it OVERWRITES the oldest entry and RETURNS it (RingDeque's substrate with its full-push policy INVERTED), the zero-GC answer to the `push`-then-`shift`-on-full Array log's O(n) trap. And **CuckooMap**, a bounded-probe WORST-CASE O(1)-lookup exact map from GENERAL INTEGER keys (`|k| <= 2^53`) to numbers over a bucketized cuckoo table (2 tables x 4 slots, `get` / `has` / `delete` probe at most 8 slots) -- the family's first general-key exact dictionary, O(capacity) space over a sparse / large integer key domain (vs SparseSet's O(universe) dense one), whose amortized `set` wears an honest in-place re-seed spike, the zero-GC answer to a general hash map's average-case-only lookup and its GC. And **SparseTable**, a WORST-CASE O(1)-QUERY STATIC range-minimum / range-maximum table (the idempotent-operation sparse table / "StaticRMQ") over two immutable `Float64Array` columns (a source copy + a flat `n*(K+1)` table) -- the family's FIRST static build-once / immutable member: build once, then answer the min OR max over any range `[l, r]` in worst-case O(1) (a floor-log2 + two table reads + one compare), the zero-GC answer to a naive O(len) range-scan (the O(n log n) build + table space are a disclosed co-headline). And **BitSet**, a WORST-CASE O(1) fixed-capacity multi-word DENSE bitset over MANY `Uint32` words (N >> 32) plus a 3-level popcount summary -- `test` / `set` / `unset` / `toggle` are one word load + one mask op, and `firstSet` / `nextSet` are worst-case O(1) via the summary (a fixed <= 32-word top scan + a 3-hop `clz32`/`ctz32` descent, never an O(words) scan), with in-place O(words) bulk `and` / `or` / `xor` / `andNot`; the canonical membership / flag structure for visited sets, dirty masks, replay windows, and permission bitmaps at scale, the zero-GC answer to a cache-degrading `Set<number>` (and the multi-word sibling of `@zakkster/lite-fastbit32`'s single 32-flag word, design-parity only, zero runtime dep). And **AliasTable**, a WORST-CASE O(1) STATIC Vose weighted sampler over two immutable typed-array columns (`_prob` Float64 + `_alias` Uint32, plus an owned weight copy) -- build a table from a fixed weight vector once (an O(n) precompute), then `sample()` draws an outcome index in `[0, n)` by WEIGHT in worst-case O(1) (two per-instance LCG advances + one compare + one read, independent of n and the weight skew), the family's SECOND static build-once / immutable member and the WEIGHTED complement to RandomSet's uniform draw, the zero-GC answer to a naive O(n) cumulative-scan sampler (the O(n) build + 2n typed-array space are a disclosed co-headline). And **CoarseTimerWheel**, a WORST-CASE O(1) near-unbounded NON-CASCADING timing wheel (the Linux 4.8 timer-wheel-rework shape: 9 levels x 64 coarse buckets, delay range `62 x 2^24`, an 18-word non-empty-bucket bitmap) over the TimerWheel substrate -- `schedule` / `cancel` / `advance` / `drainDue` / `peekNext` / `fireTimeOf`, the THIRD timing wheel: a far-future timer sits in a coarse bucket and fires IN PLACE (never cascaded), so every op is worst-case O(1) with NO max-single-op line (the difference from HierarchicalTimerWheel's cascade spike), trading PRECISION for range -- an APPROXIMATE, bounded one-sided-late fire time (`< 12.5%`, level 0 exact) is the disclosed co-headline, the zero-GC answer to a 4-ary-heap timer queue's O(log n) per op when the delay horizon is wide and exact firing is not required. And **WindowFold**, a WORST-CASE O(1) GENERAL FIFO sliding-window aggregator (DABA-Lite -- the De-Amortized Banker's Aggregator, IBM Research) over two `Float64Array` columns (raw value + partial aggregate) -- `push` / `evict` / `query` are each worst-case O(1) (<= 2 combines, no window-size branch) for ANY of four frozen associative operators (SUM / MIN / MAX / PRODUCT, a monoid), the general complement to MonoDeque's amortized min/max: it de-amortizes the two-stacks flip one step per op, so there is NO max-single-op spike, and `query()` on an empty window returns the operator identity, the zero-GC answer to a naive O(W) full-window refold. And **RankSelect**, a WORST-CASE O(1) STATIC succinct rank/select bitvector index over a frozen bit pattern (cs-poppy: 512-bit basic blocks + a select sampling layer) built from a RAW WORD ARRAY + an explicit `nbits` (copied into a private `Uint32Array`, not a BitSet instance) -- `rank1` / `rank0` count set / clear bits before an index, `select1` / `select0` return the position of the k-th set / clear bit, `access` reads a bit, all in worst-case O(1) (a fixed directory lookup + a bounded `<= 16`-word block scan), the family's THIRD static build-once / immutable member and the succinct primitive the ecosystem was missing (the substrate of wavelet trees, succinct tries, and monotone integer sets; `@zakkster/lite-loglogn` re-adopts it), the zero-GC answer to a naive O(words) popcount scan (the O(n) build + `~3.2%` index space are a disclosed co-headline). And **EliasFano**, a zero-GC succinct STATIC encoding of a monotone non-decreasing integer sequence (quasi-succinct, ~2 + log2(U/n) bits/element) built on a composed RankSelect over the upper bits -- `access(i)` returns the i-th value in worst-case O(1), while `nextGEQ(x)` (successor-or-equal) is data-dependent O(1) typical / O(log n) worst-case (an in-bucket binary search after an O(1) select0 seek), the family's first data-dependent op; the succinct primitive behind FM-indexes and compressed monotone integer lists (SDSL, folly), the near-optimal-space answer to storing a large sorted integer set with O(1) random access. And **Reservoir**, a WORST-CASE O(1)-per-item exact uniform k-sampler over an UNBOUNDED stream (Vitter's Algorithm R) over one fixed `Float64Array` of k slots -- `add(v)` keeps a uniform-random sample of the stream in FIXED memory (the first k items are stored, then the i-th is retained with probability k/i by overwriting a uniformly chosen slot: one LCG advance + one compare + a conditional store), so it prints NO max-single-op line; the STREAMING complement to RandomSet's materialized-set draw and AliasTable's static weighted draw, the zero-GC answer to buffering the whole stream in a growing array just to sample it. And **WindowFoldUint32**, the bitwise / masking window engine: a WORST-CASE O(1) GENERAL FIFO sliding-window fold over a frozen BITWISE operator (`OR` / `AND` / `XOR`) on 32-bit MASKS, over two `Uint32Array` columns (the same DABA-Lite de-amortized core as WindowFold) -- `push` / `evict` / `query` are each worst-case O(1) (<= 2 combines, a single ALU `|`/`&`/`^`, no flip spike), for union / intersection / parity of component masks, state flags, and permission sets over a sliding window. It is its OWN class, not a WindowFold operator flag: a `Float64` aggregate lane cannot honestly carry 32-bit `& | ^` (they coerce to 32-bit; AND's all-ones identity has no clean Float64 form), so register width forces a separate typed member -- data type dictates structure. The mask is a STRICT uint32 (fail closed, never coerced); `query()` returns an unsigned uint32, the operator identity on an empty window. They share no mutable module state, so a bundler that imports one drops the others.
...
Every op above is O(1) worst-case and allocates zero bytes after construction. The `witness` harness (`npm run witness`) witnesses SparseSet holding its ops/ms from n=1e3 to n=1e7 while a native `Set` falls off a cliff.
...
- [AliasTable](#aliastable)
  - [How AliasTable works](#how-aliastable-works)
  - [AliasTable API reference](#aliastable-api-reference)
...
1. **The Big-O claim is never checked on your host.** A library says "O(1)" and you take it on faith. But a real engine can turn a nominal O(1) into something that decays with `n`: a hash set's buckets scatter across an ever-larger table until every lookup is a cache miss. `lite-o1`'s analytical anchor is **throughput invariance** -- ops/ms that stays FLAT as `n` grows across orders of magnitude. That flat line IS the empirical witness of the constant, and the shipped witness reports it as a number and a shape, against a built-in `Set` foil on the identical sweep.
...
Existing options: a native `Set` (arbitrary keys, but a hash table that decays and an O(n) clear), a plain `Array` of flags (O(1) set/test but O(n) clear and O(universe) iterate), or roll-your-own (and get the delete-swap back-pointer wrong). `lite-o1` is the zero-GC primitive for a dense integer domain, with the witness attached.
...
Every query / merge above is O(1)-amortized and zero-allocation after construction. Fail closed: a bad element (non-integer, out of `[0, n)`, `NaN`, `null`, a Symbol, a BigInt) throws a `[lite-o1]` error -- never a raw `TypeError`, and `null` is never coerced to element `0`. The `witness` harness witnesses UnionFind's amortized `find` holding its ops/ms while a naive disjoint-set (no path compression, no union-by-size) collapses as `n` grows.
...
Together these bound any single op at O(alpha(n)) AMORTIZED. **Honesty:** a single `find` is NOT worst-case O(1) -- an adversarial chain that has not yet been halved is O(depth). The guarantee is amortized alpha(n) (effectively a small constant), and the [witness](#the-o1-witness) shows the amortized throughput stay flat while a naive disjoint-set foil (no compression, no union-by-size -> a degenerate chain) decays toward O(n).
...
Every `push` / `evictOlderThan` is O(1)-amortized and zero-allocation after construction; `value()` / `frontSeq()` are O(1) front reads that return `undefined` on empty (never throw). A push on a full ring throws a `[lite-o1]` error as a byte-identical no-op -- fail closed, no silent drop. For BOTH the min and the max of the same stream, run two MonoDeques (`kind` is frozen per instance). The `witness` harness witnesses MonoDeque's amortized push holding its ops/ms while a full-window rescan collapses as the window grows:
...
Both `sample()` and `removeRandom()` are WORST-CASE O(1) and zero-allocation after construction; both return `undefined` on an empty set (never throw). The `witness` harness witnesses `sample()` holding its ops/ms while a native `Set` that iterates-to-the-kth to pick uniformly collapses as the set grows:
...
Every `insert` / `decreaseKey` / `extractMin` / `peekMin` is amortized O(1) and zero-allocation after construction; `extractMin()` / `peekMin()` return `undefined` on empty (never throw), and `priorityOf()` returns `-1` for an absent key. The MONOTONE contract is what buys the constant: the extract order is non-decreasing and the cursor never rewinds, so an `insert` (or `decreaseKey`) to a priority below the cursor throws `[lite-o1]` fail-closed. The `witness` harness witnesses `extractMin` holding its ops/ms while an alloc-free binary min-heap on the same monotone trace runs measurably slower per op:
...
**Worst-case, not amortized.** Unlike BucketQueue (a cursor that can jump O(gap)) or MonoDeque (a push that can pop a run), TimerWheel has no amortized asterisk: the drain-before-advance guarantee keeps a slot to one rotation's timers, so `schedule` / `cancel` / `advance(1)` are each a fixed number of pointer writes. The `now` counter is a plain double capped at 2^53 via a `>=` ceiling guard (`advance` past it throws rather than lose the integer-exactness the `(now + delay) & MASK` slot math needs). For unbounded delays, CoarseTimerWheel (the near-unbounded, approximate-fire wheel) is the right tool -- a simple wheel is FOR a bounded delay horizon.
...
## AliasTable
<details>
<summary>A worst-case-O(1) STATIC Vose weighted sampler: build once from a fixed weight vector, then draw an outcome index by weight in O(1) -- the weighted complement to RandomSet's uniform draw.</summary>

```js
import { AliasTable } from '@zakkster/lite-o1';

// a loot table: weights need not sum to 1 (they are normalized at build)
const loot = new AliasTable([50, 30, 15, 4, 1]); // common ... legendary

loot.sample();        // an outcome index in [0, 5), drawn BY WEIGHT -- worst-case O(1)
loot.weightOf(4);     // 1     -- the original input weight of outcome 4 (O(1), never throws)
loot.size;            // 5     -- number of outcomes

// reproducible streams: a per-instance seed
const a = new AliasTable([1, 2, 3], 12345);
const b = new AliasTable([1, 2, 3], 12345);
a.sample() === b.sample(); // true -- same seed, same weights, identical sequence
a.clear();                 // reset the PRNG to the seed; the stream restarts exactly

new AliasTable([0, 0, 0]); // throws [lite-o1] -- needs at least one strictly-positive weight
new AliasTable([1, -1]);   // throws [lite-o1] -- a negative weight fails closed (before any build)
```

`AliasTable` is the family's **weighted random sampler** -- the O(1) complement to RandomSet (which draws UNIFORMLY). It is the suite's SECOND static build-once / immutable member (after SparseTable): building from a fixed weight vector is an **O(n)** precompute (Vose's algorithm), after which every `sample()` is **worst-case O(1)** -- two per-instance LCG advances + one `Float64` compare + one `Uint32` read, INDEPENDENT of n and of the weight distribution. Loot tables, weighted load-balancing, weighted Monte-Carlo, and procedural generation all reach for this and otherwise hand-roll an O(n) cumulative scan per draw. It joins the worst-case cohort: the flat `sample` line IS the claim, so there is NO max-single-op line.

**The build is a disclosed co-headline.** The O(n) build and the `2n` `Float64`/`Uint32` table SPACE are paid ONCE at construction and EXCLUDED from the per-op claim (the same honesty shape as SparseTable's O(n log n) build). Vose's algorithm partitions the scaled weights into small (`< 1`) / large (`>= 1`) worklists over ONE pre-allocated `Int32Array` scratch (a small stack from the front, a large stack from the back -- no per-step allocation), filling `_prob` (per-column accept probability) and `_alias` (per-column fallback outcome). The caller's weights are COPIED into an owned `Float64Array` (immutable -- a later mutation of your array can never change an already-built table; the SparseTable copy-not-reference discipline).

**Reproducible, zero-GC randomness.** The PRNG is a per-instance Numerical-Recipes LCG, DUPLICATED inline (RandomSet's idiom) so there is no shared mutable module state and tree-shaking stays intact. Each `sample()` runs two advances: the first word's HIGH bits pick a column (`floor(s / 2^32 * n)`, not `s % n` -- the LCG's low bits are weak), the second word's high bits give a uniform in `[0, 1)` compared against `_prob[col]`. Two default-seeded tables produce IDENTICAL sequences (a feature for reproducibility; pass distinct seeds to decorrelate). `clear()` resets the generator to the seed.

**Reach for AliasTable when** you draw from a FIXED discrete distribution repeatedly and want O(1) per draw. **Avoid it when** the weights change between draws (it is immutable -- a reweight is an O(n) rebuild, disclosed future work), you want UNIFORM sampling from a live membership set (that is RandomSet's `sample` / `removeRandom`), or you need the payload rather than an index (keep payloads in a parallel column keyed by the returned index). See [`GUIDE.md`](./GUIDE.md) for the full reach-for / avoid / measure-it.

### How AliasTable works

Vose's alias method turns a discrete distribution into two flat columns so a draw is O(1). Scale each weight to `n * p_i` (`p_i = w_i / sum`); a column with scaled weight `< 1` is "small", `>= 1` is "large". Repeatedly pair a small column `l` with a large one `g`: `l` accepts with probability `scaled[l]` and otherwise aliases to `g` (`_prob[l] = scaled[l]`, `_alias[l] = g`); `g` donates `1 - scaled[l]` of its mass and is re-filed as small or large by the remainder. Each pairing finalizes one column, so the build is O(n) over the pre-allocated worklist scratch; any float residue left on either stack is a full column (`_prob = 1`). At draw time, pick a uniform column and accept-or-alias on one compare -- worst-case O(1), no scan, no rejection loop.

### AliasTable API reference

```ts
new AliasTable(weights: ArrayLike<number>, seed?: number)
                            // weights: a real Array or numeric TypedArray, length n in [1, 2^26],
                            //   COPIED at build (immutable); every weight typeof 'number' && finite
                            //   && >= 0, at least one > 0; seed: any integer (default 0x9e3779b1)
                            // throws [lite-o1] on a non-array / empty / bad-length weights, a NaN /
                            //   +/-Infinity / negative / non-numeric element, or an all-zero vector
                            //   (typeof-first, before any table is allocated -- nothing half-built escapes)

sample(): number            // an outcome index in [0, n), drawn BY WEIGHT; worst-case O(1), never throws
weightOf(i: number): number // the original input weight of outcome i; O(1); a bad index -> 0 (never throws)
clear(): this               // reset the PRNG to the construction seed; O(1) (the table is immutable)
forEach(fn: (weight: number, index: number, table: AliasTable) => void): void  // original weights, alloc-free O(n)

get size: number            // number of outcomes n (sample() returns an index in [0, size))
get seed: number            // the RNG seed clear() resets to (uint32)
```

`sample()` is WORST-CASE O(1) and zero-allocation; the O(n) build and `2n` `Float64`/`Uint32` space are the disclosed co-headline (paid once at construction). Build-once / sample-only: there are NO mutators (no reweight -- an O(n) rebuild, disclosed future work) and NO `[Symbol.iterator]`; the only allocator is the constructor. `ALIASTABLE_MAX_N = 2^26` is the fixed outcome-count ceiling -- a TYPE bound keeping every index SMI-safe, not a size any host must allocate.
...
**AliasTable** allocates its three backing typed arrays once, at construction (`_prob` `Float64Array` + `_alias` `Uint32Array` + an owned `_w` weight copy, each `n` elements):

| Operation                        | Steady-state allocations |
| -------------------------------- | ------------------------ |
| `sample()`                       | **0** (two LCG advances + one compare + one read) |
| `weightOf(i)`                    | **0** (one guarded typed-array read) |
| `clear()`                        | **0** (resets the PRNG word; the table is immutable) |
| `forEach(fn)`                    | **0** (O(n) alloc-free scan with a hoisted callback) |
| `new AliasTable(weights, seed?)` | once, at construction (the O(n) Vose build + `_prob` / `_alias` / `_w`) |

The weight guard is the same branchless typeof-first check as the rest of the family (`typeof v !== 'number' || v !== v || v === Infinity || v === -Infinity || v < 0`) applied to every weight at build, with the `_badWeight` throw builder (using `String(v)`) on the cold path -- so a Symbol / BigInt weight fails closed rather than crashing raw, and a bad / empty / all-zero weight vector is a byte-identical no-op (the throw precedes any table allocation -- nothing half-built escapes). `sample()` is WORST-CASE O(1) (never a run, no rejection loop), so there is no amortized spike and no max-single-op line; the one-time O(n) Vose build + `2n` `Float64`/`Uint32` table space are the disclosed co-headline (paid at construction, EXCLUDED from the per-op claim). The torture and perf gates prove AliasTable at **0 B/op** across the sample scenario, with a 0-delta on all three backing buffers (build-once -- no rebuild) and the leak tracker back at `size() = 0`. `weightOf` and `forEach` allocate nothing; there is NO `[Symbol.iterator]` (sample-only, no per-element iteration protocol).

**CoarseTimerWheel** allocates its fixed backing columns once, at construction (the SparseSet cross-check id store `_dense` / `_sparse`, the intrusive per-bucket FIFO `_next` / `_prev`, `_bucketOf`, a `_fireAt` `Float64Array` read only off the hot path, the `9 x 64` bucket head/tail endpoints, and an 18-word `Uint32Array` non-empty-bucket bitmap):

| Operation                                | Steady-state allocations |
| ---------------------------------------- | ------------------------ |
| `schedule(id, delay)`                    | **0** (a <= 9-iteration level select + one tail link + one bitmap bit-set) |
| `cancel(id)`                             | **0** (unlink + conditional bit-clear) |
| `advance(ticks)`                         | **0** (bitmap find-first-set, worst-case O(1) for any `ticks`) |
| `drainDue(fn)`                           | **0** (splice the <= 9 due buckets finest-first, then head-drain; HOIST the callback) |
| `peekNext()` / `fireTimeOf(id)` / `has(id)` | **0** (guarded reads; never throw) |
| `new CoarseTimerWheel(universe, cap?)`   | once, at construction (the id columns + the 18-word bitmap) |

CoarseTimerWheel is the honest Linux-4.8 non-cascading design: a far-future timer sits in a COARSE bucket and fires IN PLACE -- never cascaded, never re-filed -- so `schedule` / `cancel` / `advance` / `drainDue` are WORST-CASE O(1) with NO max-single-op line (the difference from HierarchicalTimerWheel's cascade spike). The trade is PRECISION, not complexity: a fire time is APPROXIMATE, bounded ONE-SIDED-LATE (`now + delay <= fire < now + delay + granularity`, never early; worst-case relative error `< 12.5%`, level 0 exact) -- the disclosed co-headline. The horizon is `MAX_DELAY = 62 x 2^24` (the Linux `WHEEL_TIMEOUT_MAX` phase margin); a delay past it, a bad id / delay, or a new id past capacity throws `[lite-o1]` (typeof-first, byte-identical no-op, before any store is touched), while the queries never throw. The torture and perf gates prove CoarseTimerWheel at **0 B/op** across `schedule` / `cancel` / `advance` / `drainDue`, with a 0-delta on every backing buffer across schedule/drain cycles and the leak tracker back at `size() = 0`.

**WindowFold** allocates its two fixed backing columns once, at construction (a `_val` `Float64Array` of raw pushed values + a `_agg` `Float64Array` of partial aggregates, both sized to the rounded power-of-two capacity):

| Operation                    | Steady-state allocations |
| ---------------------------- | ------------------------ |
| `push(v)`                    | **0** (fold into the running back sum + one de-amortized flip step, <= 2 combines) |
| `evict()`                    | **0** (advance the front + one de-amortized flip step) |
| `query()`                    | **0** (<= 2 combines over stored aggregates; the operator identity on an empty window) |
| `clear()`                    | **0** (resets positions + running sum + flip state; touches no store) |
| `forEach(fn)`                | **0** (front -> back scan; HOIST the callback) |
| `new WindowFold(cap, op)`    | once, at construction (the two Float64 columns) |

WindowFold is the GENERAL FIFO sliding-window aggregator (DABA-Lite -- the De-Amortized Banker's Aggregator, IBM Research): it folds ANY of four frozen associative operators (SUM / MIN / MAX / PRODUCT, a monoid) over the live window in TRUE WORST-CASE O(1). The classic two-stacks would FLIP (reverse the back into a fresh front when the front empties) with an O(W) evict spike; DABA-Lite DE-AMORTIZES that flip one step per push/evict, so no single op does more than TWO combines and there is NO max-single-op line (unlike the amortized MonoDeque). The operator is frozen at construction (a ctor-cached int drives a switch-free combine, no operator-string test, no lambda on the hot path); `query()` on an empty window returns the operator IDENTITY (0 / +Infinity / -Infinity / 1), never undefined (`null` is not zero). Capacity rounds up to a power of two; a `push` on a full ring, a non-number / NaN value, or a bad operator name throws `[lite-o1]` (typeof-first, byte-identical no-op), while `query()` never throws. The torture and perf gates prove WindowFold at **0 B/op** across `push` / `evict` / `query`, with a 0-delta on both backing buffers across fill/slide/drain cycles and the leak tracker back at `size() = 0`. It is the general complement to MonoDeque (amortized min/max only); the bitwise AND / OR / XOR variant is deferred to a future int32-lane `WindowFoldInt32`.

**RankSelect** allocates its private data words + the cs-poppy directory once, at construction (a `_w` `Uint32Array` of `ceil(nbits/32)` bit words copied from the source + the immutable rank directory: `_l0` Float64 super-block counts, `_l1` / `_l2` `Uint32` lower-block cumulative + packed relative counts, and the two `Uint32` select sample arrays):

| Operation                    | Steady-state allocations |
| ---------------------------- | ------------------------ |
| `rank1(i)` / `rank0(i)`      | **0** (a fixed L0+L1+L2 lookup + a bounded `<= 16`-word in-block popcount scan) |
| `select1(k)` / `select0(k)`  | **0** (jump to the sampled block + a bounded block/word/bit scan; not rank + binary search) |
| `access(i)`                  | **0** (one word load + one mask) |
| `size` / `length` / `indexBytes` | **0** (O(1) getters; `size` is the precomputed popcount) |
| `forEach(fn)`                | **0** (ascending set-bit scan; HOIST the callback) |
| `new RankSelect(src, nbits)` | once, at construction (the copied words + the cs-poppy directory) |

RankSelect is the STATIC succinct rank/select bitvector index (cs-poppy -- Zhou/Andersen/Kaminsky, SEA 2013): a two-level cumulative rank directory over 512-bit basic blocks plus a select sampling layer (a sample every 8192 set bits, and every 8192 clear bits for `select0`), `~3.2%` index overhead (beating SDSL v5's 6.25%). `rank1` is a fixed directory lookup (L0 + L1 + L2) plus a bounded `<= 16`-word in-block popcount scan (the on-the-fly L3 per-word prefix, NOT a stored array), so it is TRUE WORST-CASE O(1); `select1` jumps to the sampled basic block and advances basic-block by basic-block over the directory's O(1) per-block boundary rank (a bounded forward scan seeded by the sample, never a global binary search), so it too is worst-case O(1). `rank0` / `select0` come FREE from the same directory (a block's clear count is `512*b - rank1`). It is built from a RAW WORD ARRAY + an explicit `nbits`, COPIED into a private `Uint32Array` (immutable -- rebuild to change, zero coupling to the mutable BitSet member); bits at or above `nbits` are masked to 0 and never counted. A bad `nbits` (non-integer / 0 / > 2^25 / null / Symbol) throws `[lite-o1]` BEFORE any typed array is allocated (typeof-safe), while the queries never throw (a bad `i` -> `rank` 0 / `access` undefined; a bad / overflow `k` -> `select` -1). The O(n) build + the `~3.2%` index space are the disclosed co-headline, EXCLUDED from the per-op claim, so there is NO max-single-op line. The torture and perf gates prove RankSelect at **0 B/op** across `rank1` / `select1` / `forEach`, with a 0-delta on every backing buffer (build-once -- no rebuild) and the leak tracker back at `size() = 0`. `@zakkster/lite-loglogn` re-adopts it as its rank/select substrate.

**EliasFano** allocates its packed low store + a composed RankSelect over the upper bits once, at construction (a `_low` `Uint32Array` of `ceil(n*L/32)` bit-packed low words + a `_rs` RankSelect over the `n + numBuckets`-bit unary-gap upper bitvector; `L = floor(log2(U/n))`):

| Operation                    | Steady-state allocations |
| ---------------------------- | ------------------------ |
| `access(i)`                  | **0** (one `select1` + one bit-packed low read: `(select1(i) - i) << L | low(i)`) |
| `nextGEQ(x)`                 | **0** (an O(1) `select0` bucket seek + a bounded in-bucket binary search over the low bits) |
| `size` / `length` / `universe` / `bitsPerElement` / `sizeBytes` | **0** (O(1) getters) |
| `forEach(fn)`                | **0** (ascending decode; HOIST the callback) |
| `new EliasFano(source)`      | once, at construction (the packed low store + the composed RankSelect) |

EliasFano is the STATIC succinct encoding of a monotone non-decreasing integer sequence (quasi-succinct -- Elias 1974 / Fano 1971, Vigna's quasi-succinct indices): the low `L` bits are bit-packed and the upper bits are a unary-gap bitvector read through a COMPOSED RankSelect, for `~2 + ceil(log2(U/n))` bits/element -- near the information-theoretic minimum for a sorted set. `access(i)` is TRUE WORST-CASE O(1) (one `select1` + one low read), so it joins the worst-case cohort with NO max-single-op line. `nextGEQ(x)` (successor-or-equal) is the family's first DATA-DEPENDENT op: an O(1) `select0` seek lands the target bucket, then a bounded in-bucket binary search over the sorted low bits -- O(1) TYPICAL on well-distributed keys (buckets average ~1 element at `L`), O(log n) WORST-CASE on clustered keys. It is labeled as such, never as a clean "expected O(1)" nor as worst-case O(1) ([`decisions/0025`](./decisions/0025-eliasfano.md)). Built from a SORTED numeric source COPIED into the packed store (immutable -- rebuild to change); an unsorted / negative / non-integer value, or `n` / `U` past the ceiling, throws `[lite-o1]` BEFORE any typed array is allocated (it does NOT sort internally), while the queries never throw (a bad `i` -> `access` undefined; `x` > max or a bad `x` -> `nextGEQ` -1). The O(n) build + the succinct space are the disclosed co-headline, EXCLUDED from the per-op claim. The torture and perf gates prove EliasFano at **0 B/op** across `access` / `nextGEQ` / `forEach`, with a 0-delta on every backing buffer (build-once -- no rebuild) and the leak tracker back at `size() = 0`. Sub-logarithmic predecessor/successor (O(log log U)) is `@zakkster/lite-loglogn`'s domain; arbitrary-index mutable range queries are `@zakkster/lite-logn`'s Fenwick.

**Reservoir** allocates one fixed `Float64Array` of `k` slots once, at construction (the reservoir store `_store`; the seen-count, LCG state, and seed are scalars -- nothing else is ever allocated):

| Operation                    | Steady-state allocations |
| ---------------------------- | ------------------------ |
| `add(v)`                     | **0** (one NR-LCG advance + one compare + a probability-`k/i` conditional store) |
| `get(i)`                     | **0** (one bounds check + one typed read) |
| `size` / `seen` / `capacity` / `seed` | **0** (O(1) getters) |
| `clear()` / `reset()`        | **0** (reset the seen count; `reset()` also restores the seed -- no store touched) |
| `forEach(fn)`                | **0** (scan the live sample; HOIST the callback) |
| `new Reservoir(k, seed)`     | once, at construction (the fixed `k`-slot `Float64Array`) |

Reservoir is the STREAMING uniform sampler (Vitter's Algorithm R): it keeps a uniform-random sample of `k` items drawn from a stream of unknown, unbounded length in FIXED memory. The first `k` items are stored, then for the i-th item (1-indexed) `add` retains it with probability `k/i` by overwriting a uniformly chosen slot -- one NR-LCG advance + one compare + one conditional store, never a run, so `add` is TRUE WORST-CASE O(1) and prints NO max-single-op line. The reservoir IS the sample: read it with `get(i)` / `forEach` / iterate (there is deliberately NO `sample()`). It completes the sampling trio -- RandomSet draws uniformly from a MATERIALIZED live set (stores every member), AliasTable is the STATIC WEIGHTED draw, Reservoir samples uniformly from an UNBOUNDED stream storing NOTHING but the sample. The high-bits multiply index (`floor(s / 2^32 * (n+1))`, not `s % (n+1)`) carries a residual bias `<= n / 2^32` -- DISCLOSED, not coded around (rejection sampling would break the worst-case O(1)); sampling is statistical, not cryptographic ([`decisions/0026`](./decisions/0026-reservoir.md)). `k` is fixed and EXACT (not power-of-two rounded); a bad `k` / seed throws `[lite-o1]` before any allocation, a non-clean value throws (typeof-first, byte-identical no-op), and the `2^53` seen-count ceiling throws (a `>=` guard -- past it `k/i` stops being integer-exact); `get()` never throws (a bad `i` -> undefined). `clear()` empties without reseeding (the RNG stream continues); `reset()` empties AND restores the seed so the draw sequence replays exactly. The torture and perf gates prove Reservoir at **0 B/op** across `add` / `get`, with a 0-delta on the backing buffer (fixed `k` slots -- `seen` grows but the store never does) and the leak tracker back at `size() = 0`. For a weighted stream sample (A-Res) or a sliding-window sample, neither ships -- Reservoir is the exact uniform whole-stream sampler.

**WindowFoldUint32** allocates its two `Uint32Array` columns (a raw-mask lane + a partial-aggregate lane) once, at construction -- the same DABA-Lite substrate as WindowFold, at Uint32 width:

| Operation                    | Steady-state allocations |
| ---------------------------- | ------------------------ |
| `push(mask)`                 | **0** (append + a single ALU combine `|`/`&`/`^`, `>>> 0`-normalized, over recycled typed slots) |
| `evict()`                    | **0** (one de-amortized flip step, scalar + typed-slot work) |
| `query()`                    | **0** (<= 2 combines; returns the unsigned uint32 aggregate) |
| `op` / `size` / `capacity`   | **0** (O(1) getters) |
| `clear()`                    | **0** (resets positions + running aggregate + flip state; zeroes no store) |
| `forEach(fn)`                | **0** (oldest -> newest scan; HOIST the callback) |
| `new WindowFoldUint32(capacity, op)` | once, at construction (the two `Uint32Array` lanes) |

WindowFoldUint32 is the BITWISE / MASKING window engine -- the sibling `WindowFold` couldn't be, and that is the lesson: a `Float64` aggregate lane holds integers to 2^53, but `& | ^` coerce to 32-bit and AND's all-ones identity has no clean Float64 expression, so register width (not taste) forces a separate typed member. Data type dictates structure. It is the DABA-Lite de-amortized sliding-window fold (identical six-cursor core to WindowFold) over three associative bitwise monoids: `OR` (identity `0`, union of masks), `AND` (identity `0xFFFFFFFF`, intersection), `XOR` (identity `0`, rolling parity). The operator is frozen at construction (a ctor-cached int drives a switch-free combine); `push` / `evict` / `query` are each worst-case O(1) (<= 2 combines, no window-size branch, no flip spike), so there is NO max-single-op line, and `query()` is a single ALU op (`partialA | partialB`) with zero FP. `query()` on an empty window returns the operator identity, never undefined, as an UNSIGNED uint32. The mask arg is a STRICT uint32 -- `typeof mask === 'number' && (mask >>> 0) === mask`, i.e. an integer in `[0, 2^32)`; a float, negative, `>= 2^32`, NaN, Symbol, or BigInt throws `[lite-o1]` typeof-first (byte-identical no-op). It is NEVER coerced: a 53-bit compound integer would silently strip its top bits under `>>> 0`, corrupting the aggregate, so it fails closed instead (the HierarchicalTimerWheel / Reservoir precedent). `-1` is NOT accepted as all-ones -- pass `0xFFFFFFFF`. Fixed capacity (rounds up to a power of two), fail closed on a full push ([`decisions/0027`](./decisions/0027-windowfolduint32.md)). The torture and perf gates prove WindowFoldUint32 at **0 B/op** across `push` / `evict` / `query`, with a 0-delta on both backing lanes and the leak tracker back at `size() = 0`. Non-overlap: `WindowFold` is the NUMERIC (SUM/MIN/MAX/PRODUCT, Float64) sibling; for arbitrary-index / out-of-order range aggregation use `@zakkster/lite-logn`'s Fenwick.
...
- **UnionFind is amortized, not worst-case, and honest about it.** Path halving (iterative, no stack -- so zero-alloc) plus union by size bound any single op at O(alpha(n)) amortized; a single `find` is O(depth) worst-case, and the witness shows the amortized line against a naive-disjoint-set foil. `reset()` and `forEachRoots()` are the O(n) exceptions (named `reset()`, not `clear()`, to flag the cost); `roots()` is the one allocating op. See [`decisions/0007`](./decisions/0007-unionfind-path-halving-union-by-size.md).
...
NOT a data-structure member). It profiles all twenty-one members against the JS built-in
...
scaling, and D8 workload micro-benches -- **twenty-one members x 8 dimensions = 168 cells**.
...
The "< 40% of all" claim holds for all twenty; with twenty-one members the all-member bundle
...
**0 major GC across all twenty-one.** The torture + perf gates are the authoritative
zero-alloc proof and hold **0 B/op** per steady-state op for all twenty-one members (incl.
...
- **Not an unbounded timer queue.** TimerWheel is a BOUNDED "simple" wheel: the delay range is `[0, slots-1]` (space O(slots)), and a delay `>= slots` throws. For delays far in the future or of unknown horizon, reach for CoarseTimerWheel (near-unbounded, approximate) or a binary-heap timer queue (exact). It is integer-tick (no sub-tick / floating-point deadlines), and its clock must be drained before it advances (an undrained slot throws).
...
- **Not a growable collection.** All twenty-one members are fixed-capacity: a SparseSet / RandomSet / FreqO1 / BucketQueue / TimerWheel / HierarchicalTimerWheel key or id past capacity, a CuckooMap key past the load ceiling, or a RingDeque / MonoDeque / MinStack / WindowFold / WindowFoldUint32 push on a full store, throws; a RingLog push on a full store OVERWRITES the oldest (lossy by design, never a resize), and a Reservoir `add` past capacity SAMPLES (Algorithm R -- the item replaces a uniformly chosen resident or is dropped, never a resize); UnionFind's element universe `n` and SparseTable's source are fixed at construction (SparseTable is immutable -- rebuild to change the data). This is deliberate (worst-case / amortized bounds, fail closed or lossy -- no hidden resize), not a missing feature.
...
- **Not the full roster forever.** v1.11.1 is the twenty-one-member stable API (the roster is now CLOSED): SparseSet + RingDeque + UnionFind + MonoDeque + MinStack + RandomSet + FreqO1 + BucketQueue + TimerWheel + HierarchicalTimerWheel + RingLog + CuckooMap + SparseTable + BitSet + AliasTable + CoarseTimerWheel + WindowFold + RankSelect + EliasFano + Reservoir + WindowFoldUint32. SlotPool (a free-list slot allocator with generational handles) is NOT a lite-o1 member -- it is owned by `@zakkster/lite-arena` (a component-free `Arena` is exactly that pool), so lite-o1 would only fork it. For unbounded delays with EXACT firing, reach for a binary-heap timer queue in `@zakkster/lite-logn`; CoarseTimerWheel covers the near-unbounded APPROXIMATE case.
