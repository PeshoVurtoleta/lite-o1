...
- **Not the full roster forever.** v1.6.0 is the sixteen-member stable API: SparseSet + RingDeque + UnionFind + MonoDeque + MinStack + RandomSet + FreqO1 + BucketQueue + TimerWheel + HierarchicalTimerWheel + RingLog + CuckooMap + SparseTable + BitSet + AliasTable + CoarseTimerWheel. SlotPool (a free-list slot allocator with generational handles) is NOT a lite-o1 member -- it is owned by `@zakkster/lite-arena` (a component-free `Arena` is exactly that pool), so lite-o1 would only fork it. For unbounded delays with EXACT firing, reach for a binary-heap timer queue in `@zakkster/lite-logn`; CoarseTimerWheel covers the near-unbounded APPROXIMATE case.
