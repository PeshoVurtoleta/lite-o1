...
NOT a data-structure member). It profiles all nineteen members against the JS built-in
...
scaling, and D8 workload micro-benches -- **nineteen members x 8 dimensions = 152 cells**.
...
The "< 40% of all" claim holds for all eighteen; with nineteen members the all-member bundle
...
**0 major GC across all eighteen.** The torture + perf gates are the authoritative
zero-alloc proof and hold **0 B/op** per steady-state op for all nineteen members (incl.
