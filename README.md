...
NOT a data-structure member). It profiles all sixteen members against the JS built-in
...
scaling, and D8 workload micro-benches -- **sixteen members x 8 dimensions = 128 cells**.
...
The "< 40% of all" claim holds for all sixteen; with sixteen members the all-member bundle
...
**0 major GC across all sixteen.** The torture + perf gates are the authoritative
zero-alloc proof and hold **0 B/op** per steady-state op for all sixteen members (incl.
